...
def main():
    ...
    if args.action=='maskfile2outline':
        ...
        # print(np.histogram(image2, bins=np.append(np.unique(image2), np.inf)))
        ...
        # #MOTION_AFFINE does not work well for some reason
        ...
        # Warp im2 
        # warp_matrix, _ =cv2.estimateAffine2D (im1_gray, im2_gray)
        image2_warp = cv2.warpPerspective (image2, warp_matrix, (sz[1],sz[0]), flags=cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP);
        ...
        # save to file
        # print(image2_warp.shape) %2D image
